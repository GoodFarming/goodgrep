...
impl IgnorePatterns {
   ...
   pub fn new(root: &Path) -> Self {
      ...
      let ggrep_ignore = root.join(".ggignore");
      if ggrep_ignore.exists() {
         let _ = builder.add(&ggrep_ignore);
      }

      let legacy_ignore = root.join(".smignore");
      if legacy_ignore.exists() {
         let _ = builder.add(&legacy_ignore);
      }
