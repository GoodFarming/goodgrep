...
pub enum EmbeddingError {
   ...
   #[error(
      "failed to download model file {file} from {model}: {reason}. Run 'ggrep setup' to download \
       models."
   )]
...
impl CandleEmbedder {
                  ...
                  .await
               {
                  Ok(dense_matrix) => {
                     for (i, &orig_idx) in batch_indices.iter().enumerate() {
                        ...
                        results[orig_idx] =
                           Some(HybridEmbedding { dense, colbert: Vec::new(), colbert_scale: 1.0 });
                     }
                     offset = end;
                  },
                  Err(e) => {
   ...
   async fn try_compute_dense_batch_indexed(
      &self,
      indices: &[usize],
      dense_tokenized: &[(Vec<u32>, Vec<u32>)],
   ) -> Result<Array2<f32>> {
