...
impl Chunker {
   fn is_markdown(path: &Path) -> bool {
      ...
      matches!(ext.to_ascii_lowercase().as_str(), "md" | "markdown" | "mmd")
   }

   fn markdown_heading_regex() -> &'static Regex {
      static HEADING: std::sync::LazyLock<Regex> =
         std::sync::LazyLock::new(|| Regex::new(r"^(#{1,6})\s+(.+?)\s*$").unwrap());
      &HEADING
   }

   fn chunk_markdown(content: &Str, path: &Path) -> Vec<Chunk> {
      let file_context: Str = format!("File: {}", path.display()).into();
      let stack_base = slice::from_ref(&file_context);

      let mut chunks = Vec::new();

      let mut fence_delim: Option<&str> = None;

      // Track the current heading path to provide breadcrumbs as chunk context.
      ...
      for (idx, line) in content.as_str().lines().enumerate() {
         let trimmed = line.trim_end();

         if let Some(delim) = fence_delim {
            if trimmed.trim_start().starts_with(delim) {
               fence_delim = None;
            }
            continue;
         }

         let fence_start = trimmed.trim_start();
         if fence_start.starts_with("```") {
            fence_delim = Some("```");
            continue;
         }
         if fence_start.starts_with("~~~") {
         ...
         if let Some(caps) = Self::markdown_heading_regex().captures(trimmed) {
      ...
      let total_lines = content.as_str().lines().count();

      // Optional preamble chunk before first heading.
      ...
      for (i, heading) in headings.iter().enumerate() {
         // Maintain heading breadcrumbs.
         let level_idx = heading.level.saturating_sub(1).min(5);
         heading_stack[level_idx] = Some(heading.title.clone());
         for deeper in (level_idx + 1)..6 {
         ...
         if next_start <= heading.start_line {
            continue;
         }

         let (start_byte, end_byte) =
            Self::line_range_to_byte_range(content, heading.start_line, next_start);
         let section = content.slice(start_byte..end_byte);
         if section.trim().is_empty() {
            continue;
         }

         let mut ctx: Vec<Str> = Vec::with_capacity(1 + 6);
         ctx.push(file_context.clone());
         for (lvl, entry) in heading_stack.iter().enumerate() {
            if let Some(title) = entry {
               ctx.push(Str::from_string(format!("Heading{}: {}", lvl + 1, title)));
            }
         }
      ...
      if headings.is_empty() {
         return Self::simple_chunk(content, path);
      }
   ...
   async fn chunk_with_tree_sitter(
      ...
      if !saw_definition {
         // No semantic "definitions" found (common for docs/config/plan files).
         // Fall back to simple chunking so these files remain searchable.
         return Ok(None);
      }
