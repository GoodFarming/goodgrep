...
Example:
```bash
ggrep "how are plugins loaded"
ggrep "how are plugins loaded" packages/transformers.js/src
```
...
1. **Search Broadly First:** Use a conceptual query to map the landscape.
   * `ggrep "authentication authorization checks"`
...
- **Trust the Semantics:** You don't need exact names. `ggrep "how does the server start"` works better than guessing `ggrep "server.init"`.
...
- **Scope When Possible:** Use path constraints to focus: `ggrep "auth" src/server/`
